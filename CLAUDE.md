...
**Prefer a library reference over vendored artwork.** `meta.json` carries icons as well as colour — `"icons": {"<nodeType>": "lucide:Send"}` per node type, or `"icon"` for the whole folder (the same specificity pair as `icon_<nodeType>.svg` / `icon.svg`). Ship an SVG only for artwork no library has, typically a brand mark. Two traps: a **file always beats a meta.json ref**, so a folder shipping `icon.svg` shadows refs for every node type in it (this is why the four WhatsApp Business nodes once rendered one glyph); and `lucide:` names are matched against the package's **export** identifiers, so `lucide:CheckCheck` resolves and the kebab-case `lucide:check-check` silently renders nothing. Locked from both sides — `client/src/assets/icons/index.test.ts` walks every `meta.json` under `server/nodes` and asserts each ref resolves to a real component; `test_node_spec.py` asserts every `icons` key is a registered node type.
...
The journal records, it never reconstructs. `agent.execute_llm_step` writes each turn from the exact message list it hands to `ChatUnifier.chat` (`_journal_llm_turn`), immediately after the call returns. `agent.prepare_context` deliberately journals nothing — it runs before the request exists, so anything it wrote would be assembled from configuration. Journal operation ids derive from the per-firing `context_execution_id`, **not** `execution_id`: the latter is generation-scoped, so every message in a generation minted identical ids and the store's idempotency guard silently discarded every turn after the first. They are **also scoped by `agent_node_id`** — two agents wired to one Context node resolve to the same thread, so a firing-only id made them collide on `(thread, operation_id)` and one agent's turns vanished as replays.

**The one place the journal is read back is the `continue_as_new` rollover.** `AgentWorkflow` rolls over under history pressure *only when `context_ref` is set* (`agent_workflow.py`), and a rollover carries references, not the conversation — so the resumed run rebuilds `messages` from system + memory + prompt alone. Without a replay step that means attaching a Context node is what makes a long run silently forget everything it had said: the same class of bug as the original `context_ref` regression, one boundary later. The resumed run therefore calls `agent.reconstruct_context_messages` (read-only, wraps `reconstruct_message_wire_v2`) before its first LLM step. That activity **degrades instead of raising** — an opaque provider checkpoint or an unreadable thread returns `restored: False` with a reason and the run continues from the rebuilt prompt, because a rehydration miss must never turn an unreadable thread into a dead agent. Locked by `tests/temporal/test_agent_workflow.py::TestContextJournalIdentity`, which also asserts the workflow source carries no comment claiming the LLM activity rebuilds its request from the store — that sentence was false for a long time and is exactly what a future reader would re-implement.
...
**Live viewing is emitted at the persistence boundary, not per caller.** `AgentContextStore` is the one place every writer passes through (in-process loop, Temporal LLM activity, CLI-agent bridge), so it announces durable commits through a fanout registry in [`services/agent_context/listeners.py`](./server/services/agent_context/listeners.py) — `register_context_commit_listener` / `notify_context_commit` — and `nodes/context/__init__.py` registers the broadcaster. Same shape as the plugin registries, same reason: the store must never import `nodes/`. A new writer gets live updates for free. Three properties are load-bearing: the notification fires **after commit and after the ref reload** (never ahead of the state it describes); the **idempotent-replay path emits nothing** (a retried activity is not a state change); and a listener **can never fail a commit** — these commits run inside the Temporal LLM activity's heartbeat-silent post-send window under a 60 s `heartbeat_timeout` on a `maximum_attempts=1` policy, so a throwing or slow listener would fail a run over a UI notification. Epoch rotation is deliberately not routed through the store: `start_epoch`'s callers already emit `context.epoch.started` with a `reason` the store cannot know.

The three Context events (`context.updated` / `context.compacted` / `context.epoch.started`) **broadcast directly via `get_status_broadcaster()`, not through `services.events.dispatch.emit`** — `emit` runs a Temporal Visibility query to find workflow consumers, and no node type registers a canary consumer for `com.opencompany.context.*`, so that query is guaranteed to match nothing once per journal append. Payloads are identity + revision only; the panel refetches through the authorized `get_agent_context` handler, so the broadcast grants no read access on its own. Frontend consumption follows the `skill_lifecycle` precedent: three `case` labels in `WebSocketContext.tsx` invalidate `['agentContext']`, and the panel itself contains **no** subscription code. See [event_framework.md → UI-only lifecycle events](./docs-internal/event_framework.md).
