...
| **[Sarvam AI Service](./docs-internal/sarvam_service.md)** | Sarvam AI (Indic-first) — `sarvamChatModel` registered as an **OpenAI-compat provider** (one `_COMPAT_PROVIDERS` entry + a JSON block; no provider subclass) plus three text REST nodes under `nodes/sarvam/` (`sarvamTranslate` / `sarvamTransliterate` / `sarvamDetectLanguage`, palette group `language`). Sarvam's speech endpoints are NOT nodes here — they are one provider inside the abstracted `textToSpeech` / `speechToText` nodes (see **[Speech Provider RFC](./docs-internal/speech_provider_rfc.md)**). **One stored key spans two auth styles** — the chat route takes `Authorization: Bearer` (what the openai SDK sends) while every other endpoint takes only `api-subscription-key`, so `SarvamCredential` declares the native header and every surface shares it. Introduces the generic **`supports_model_listing: false`** flag: Sarvam ships no `/v1/models` route, and unhandled that 404 becomes a `NodeUserError` that `AIService.fetch_models` re-raises *before* its curated fallback, breaking key validation and the model dropdown — so `OpenAIProvider.fetch_models` serves the curated list and probes with a one-token completion instead (defaults `true`; the other 12 providers are untouched, locked by test). Reasoning needs zero Python (`thinking_type: "effort"` + `reasoning_content` are already handled); `thinking_default_on` and `reasoning_models` are deliberately omitted. Also added generic **`Connection.request(files=)`** multipart support (threaded through the auth-retry rebuild), which the speech plugin still relies on. |
...
| **[Media Transport](./docs-internal/media_transport.md)** | How files move through the engine, and why they move as **references rather than bytes**. `services/media/` — `AudioRef` (~400 B, `extra="forbid"` so adding a bytes field is a `ValidationError` rather than a silent regression), `write_audio` / `resolve_media` / `coerce_file_param` (the one nodes should call — accepts an `AudioRef`, a bare path, or the legacy base64 envelope), `inspect_audio` (tinytag → stdlib `wave` → PCM arithmetic, **never raises**), and `limits.py`. Containment runs through `resolve_within`, which closed a live vulnerability: a node joined a user path onto the workspace root unchecked, so `audio_file="../../credentials.db"` read the encrypted credential store and uploaded it. **The id/slug asymmetry is the trap** — workspace dirs are named by the mutable `Workflow.slug` while an `AudioRef` stores the immutable `workflow_id` so refs survive rename; `workspace_root()` raises rather than guessing, and the HTTP route owns the lookup. HTTP surface in [`routers/workspace.py`](./server/routers/workspace.py): Range support comes free from Starlette's `FileResponse`, and only `audio/ image/ video/` render inline — `text/html` and `image/svg+xml` are forced to `attachment` because `shell` / `fileDownloader` / `fileModify` can write arbitrary files into a workspace. |
...
│   ├── refs.py              # AudioRef — a reference, never bytes; extra="forbid" makes that structural
...
> The authoritative, per-node reference is the backend plugin registry plus the per-node "logic-flow" cards under [docs-internal/node-logic-flows/](./docs-internal/node-logic-flows/) (one card per node, grouped by category, with handles / params / outputs / side-effects / edge-cases). Live node list = the plugin registry (one folder per node under `server/nodes/<group>/`, 131 node types today via `len(services.node_registry.NODE_METADATA)`; a bare `__init__.py` glob overcounts by also matching the group packages). Do NOT maintain a per-node catalogue here — it drifts on every plugin add.

Node groups (palette categories): agent, model, skill, tool, trigger, workflow, search, google, android, whatsapp, telegram, twitter, social, email, proxy, chat, scheduler, text, code, document, location, utility, browser, scraper, filesystem, stripe, vercel, github (palette group `vcs`), cloudflare and gcloud (palette group `deployment`), sarvam (palette group `language` — translate / transliterate / detect-language, sharing one API key with `sarvamChatModel`), speech (palette group `language` — provider-abstracted `textToSpeech` / `speechToText`). See [docs-internal/node-logic-flows/](./docs-internal/node-logic-flows/) for the card index.
...
- `GET /api/workspace/{workflow_id}/files/{path:path}` - Serve a file from a workflow's workspace.
  ...
  (a distinct status would confirm what exists outside the workspace). Only `audio/ image/ video/`
  render inline — `text/html` and `image/svg+xml` are forced to `attachment`, because nodes can
  write arbitrary files into a workspace and inline markup from the app origin is stored XSS.
...
- Node execution dispatches every registered plugin via the `BaseNode` registry (one self-contained folder per node under `server/nodes/<group>/`; 131 node types today via `len(services.node_registry.NODE_METADATA)` — a bare `__init__.py` glob overcounts by also matching the group packages)
...
- The `fileRead`, `fileModify`, `fsSearch`, and `shell` nodes use the native `WorkspaceBackend` rooted at `workspace_dir`; Coding Agent can invoke those nodes when they are connected as tools
...
- **Media leaves a node as a reference, never as bytes.** `services/media/` + `AudioRef` (~400 B, `extra="forbid"`). A node result is persisted 3x, broadcast 2x, retained in `_status` forever, aggregated into the workflow result, copied into every downstream activity *input*, and — if `usable_as_tool` — serialized into an LLM message; a 12 MB base64 payload therefore fails the activity, **retries 3x re-billing the provider each time**, and reports a generic error. Nodes call `write_audio` (out) and `coerce_file_param` (in); both enforce containment through `resolve_within`, which closed a live traversal that let `audio_file="../../credentials.db"` read the encrypted credential store and upload it. **id/slug trap:** workspace dirs are named by the mutable `Workflow.slug` while an `AudioRef` stores the immutable `workflow_id` so refs survive rename — `workspace_root()` raises rather than composing a wrong path, and `routers/workspace.py` owns the lookup because it needs the DB. Full reference: **[Media Transport](./docs-internal/media_transport.md)**.
