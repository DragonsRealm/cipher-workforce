"""Contract for the File Manager listing surface.
...
from nodes.filesystem.file_manager._service import (
...
class TestNodeExecution:
    async def test_emits_file_refs_for_downstream_nodes(self, workspace):
        ...
        from nodes.filesystem.file_manager import FileManagerNode, FileManagerParams
        from types import SimpleNamespace
        ...
        result = await FileManagerNode().list_files(
            ctx, FileManagerParams(path="audio")
        )
    ...
    async def test_selection_reports_missing_without_failing(self, workspace):
        ...
        result = await FileManagerNode().list_files(
            ctx,
            FileManagerParams(selection=["audio/greeting.wav", "audio/gone.wav"]),
        )
    ...
    async def test_selection_that_is_entirely_missing_is_a_user_error(self, workspace):
        ...
        with pytest.raises(NodeUserError):
            await FileManagerNode().list_files(
                ctx, FileManagerParams(selection=["nope.wav"])
            )
    ...
    async def test_output_stays_well_under_the_temporal_warning_budget(self, tmp_path):
        ...
        result = await FileManagerNode().list_files(ctx, FileManagerParams(limit=1000))
