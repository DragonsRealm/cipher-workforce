"""File Manager — browse the workflow's workspace from the editor.
...
class FileManagerNode(ActionNode):
    type = "fileManager"
    ...
    ui_hints = {"isFileManagerPanel": True, "hideInputSection": True}

    Params = FileManagerParams
    Output = FileManagerOutput

    @Operation("list")
    async def list_files(self, ctx: NodeContext, params: FileManagerParams) -> Any:
    ...
    async def _emit_selection(
        self, ctx: NodeContext, params: FileManagerParams, workspace_dir: str
    ) -> Any:
